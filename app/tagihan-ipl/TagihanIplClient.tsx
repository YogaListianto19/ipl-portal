...
export function TagihanIplClient({ invoices, blok }: Props) {
  ...
  return (
    <main className="max-w-xl mx-auto px-4 py-5 pb-24 lg:pb-8 space-y-5">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-linear-to-br from-rose-900 via-rose-700 to-pink-600 p-5 shadow-md shadow-primary/20">
        ...
        <p className="text-white/65 text-sm font-medium relative">Tagihan IPL</p>
        <h1 className="text-xl font-bold text-white relative leading-tight">Unit {blok}</h1>
        <p className="text-white/55 text-sm mt-1 relative">Iuran Pemeliharaan Lingkungan</p>
      </div>
        ...
        allClear ? (
          ...
          <div className="flex items-center gap-3 p-4 bg-red-50 rounded-2xl border border-red-200/70">
            ...
            <div>
              ...
              <p className="text-xs text-red-600 mt-0.5">Tunggakan: <span className="font-bold">{formatRupiah(summary.totalDue)}</span></p>
            </div>
          </div>
        ) : null
      )}
      ...
      {summary.total > 0 && (
        <div className="bg-card rounded-2xl border border-border/50 shadow-sm p-4">
          ...
          <div className="h-2 bg-muted rounded-full overflow-hidden">
            <div
              className={cn('h-full rounded-full transition-all duration-500', paidPct === 100 ? 'bg-emerald-500' : paidPct >= 70 ? 'bg-primary' : 'bg-amber-500')}
              style={{ width: `${paidPct}%` }}
            />
          </div>
      ...
      {summary.totalPaid > 0 && !allClear && (
        <div className="flex items-center gap-2 p-3 bg-emerald-50 rounded-xl border border-emerald-100">
          ...
          <p className="text-sm text-emerald-700">Total dibayar: <span className="font-bold">{formatRupiah(summary.totalPaid)}</span></p>
        </div>
      )}

      {/* Invoice list */}
      <div>
        <h2 className="text-base font-bold text-foreground mb-3">Riwayat Tagihan IPL</h2>
        {invoices.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground">
            <CalendarDays className="w-10 h-10 mx-auto mb-3 opacity-30" />
            <p className="text-sm">Belum ada data tagihan IPL</p>
          </div>
        ) : (
            ...
            {byYear.map(({ year, invoices: yearInvoices }) => (
              <div key={year}>
                ...
                <div className="space-y-2">
                  {yearInvoices.map(inv => <InvoiceRow key={inv.id} invoice={inv} />)}
                </div>
              </div>
            ))}
          </div>
...
function InvoiceRow({ invoice: inv }: { invoice: Invoice }) {
  ...
  return (
    <div className={cn(
      ...
      isPaid ? 'bg-card border-border/40 hover:border-border/70' : 'bg-red-50/70 border-red-200/70 hover:border-red-300'
    )}>
      <div className={cn('w-9 h-9 rounded-xl flex items-center justify-center shrink-0', isPaid ? 'bg-emerald-100' : 'bg-red-100')}>
        {isPaid ? <CheckCircle2 className="w-4 h-4 text-emerald-600" /> : <XCircle className="w-4 h-4 text-red-500" />}
      </div>
      ...
      <div className="text-right shrink-0">
        ...
        <Badge
          ...
          className={cn('text-[10px] h-4 px-1.5 mt-0.5', isPaid && 'bg-emerald-100 text-emerald-700 hover:bg-emerald-100')}
        >
          {isPaid ? 'Lunas' : 'Belum'}
        </Badge>
      </div>
